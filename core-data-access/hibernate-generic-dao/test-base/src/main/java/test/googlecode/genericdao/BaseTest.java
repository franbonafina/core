/*
 * Copyright 2013 David Wolverton Licensed under the Apache License, Version 2.0 (the "License");
...
public abstract class BaseTest extends PersistableTestDataModel {
    ...
    protected void assertListEqual(Person[] expected, List<Person> actual) {
        Assert.assertEquals("The list did not have the expected length", expected.length,
                        actual.size());
    ...
    protected void assertArrayEqual(Object[] actual, Object... expected) {
        Assert.assertEquals("The array did not have the expected length", expected.length,
                        actual.length);
